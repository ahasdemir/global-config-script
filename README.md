...
2. (Optional, but recommended) 
   - Replace the `C:\Users\[username]\AppData\Roaming\PrismLauncher\global-config-script\options.txt` file with the backup 
   - from `C:\Users\[username]\AppData\Roaming\PrismLauncher\global-config-script\backup-original-for-avoid-corruptions\options.txt`.
